...
async def init_admin_user():
    try:
        ...
        # 检查管理员用户是否已存在
        if await user_repository.exists_by_email(settings.admin_email):
        ...
        try:
            await user_repository.create(req)
        ...
        # 获取用户ID
        user = await user_repository.get_by_email(settings.admin_email)
        user_id = user.id

        # 初始化默认连接
        database_connection_repository = DatabaseConnectionRepository()
        if not await database_connection_repository.get_by_name("opentenbase"):
            ...
            await database_connection_repository.create(req)
        ...
        if not await model_connection_repository.get_by_name(settings.model):
            ...
            await model_connection_repository.create(req)
        ...
        if not await db_schema_repository.get_by_name("tpch"):
            ...
            if schema_path:
                ...
                await db_schema_repository.create(req)
