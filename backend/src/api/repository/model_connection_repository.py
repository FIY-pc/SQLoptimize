...
class ModelConnectionRepository:
    ...
    async def create(self, model_connection_data: CreateModelConnectionReq) -> ModelConnection:
        """创建模型连接"""
        try:
            async with get_async_db() as db:
                ...
                await db.commit()
                await db.refresh(model_connection)
    ...
    async def get_by_id(self, connection_id: int) -> Optional[ModelConnection]:
        """根据ID获取模型连接"""
        try:
            async with get_async_db() as db:
                result = await db.execute(
                    select(ModelConnection).where(ModelConnection.id == connection_id)
                )
                return cast(Optional[ModelConnection], result.scalar_one_or_none())
        except Exception as e:
            logger.error(f"根据ID获取模型连接失败: {e}")
            raise

    async def get_by_name(self, model_name: str) -> Optional[ModelConnection]:
        """根据模型名称获取模型连接"""
        try:
            async with get_async_db() as db:
                result = await db.execute(
                    select(ModelConnection).where(ModelConnection.model_name == model_name)
                )
                return cast(Optional[ModelConnection], result.scalar_one_or_none())
        except Exception as e:
    ...
    async def get_by_user_id(self, user_id: int, skip: int = 0, limit: int = 100) -> List[ModelConnection]:
        """根据用户ID获取模型连接列表（分页）"""
        try:
            async with get_async_db() as db:
                result = await db.execute(
                    select(ModelConnection)
                    .where(ModelConnection.user_id == user_id)
                    .offset(skip)
                    .limit(limit)
                )
                return cast(List[ModelConnection], list(result.scalars().all()))
        except Exception as e:
            logger.error(f"根据用户ID获取模型连接列表失败: {e}")
            raise

    async def get_all(self, skip: int = 0, limit: int = 100) -> List[ModelConnection]:
        """获取所有模型连接（分页）"""
        try:
            async with get_async_db() as db:
                result = await db.execute(
                    select(ModelConnection).offset(skip).limit(limit)
                )
                return cast(List[ModelConnection], list(result.scalars().all()))
        except Exception as e:
    ...
    async def update(self, connection_id: int, connection_data: dict) -> Optional[ModelConnection]:
        """更新模型连接信息"""
        try:
            async with get_async_db() as db:
                result = await db.execute(
                ...
                connection: Optional[ModelConnection] = result.scalar_one_or_none()
                if not connection:
                    return None
        ...
        try:
            async with get_async_db() as db:
                ...
                users_result = await db.execute(
                    select(User).where(User.active_model_connections == connection_id)
                )
    ...
    async def delete_by_name(self, model_name: str) -> bool:
        """根据模型名称删除模型连接"""
        try:
            async with get_async_db() as db:
                result = await db.execute(
                ...
                # 先清除所有用户对该连接的活跃引用
                users_result = await db.execute(
                    select(User).where(User.active_model_connections == connection.id)
                )
                users_with_active_connection = users_result.scalars().all()
                for user in users_with_active_connection:
                    user.active_model_connections = None
    ...
    async def exists_by_name(self, model_name: str) -> bool:
        ...
        try:
            async with get_async_db() as db:
                ...
                return cast(Optional[ModelConnection], result.scalar_one_or_none()) is not None
        except Exception as e:
            logger.error(f"检查模型名称是否存在失败: {e}")
            raise

    async def get_by_model(self, model: str) -> List[ModelConnection]:
        """根据模型名称获取模型连接列表"""
        try:
            async with get_async_db() as db:
                result = await db.execute(
                    select(ModelConnection).where(ModelConnection.model == model)
                )
                return cast(List[ModelConnection], list(result.scalars().all()))
        except Exception as e:
    ...
    async def get_by_user_and_model(self, user_id: int, model: str) -> List[ModelConnection]:
        """根据用户ID和模型类型获取模型连接列表"""
        try:
            async with get_async_db() as db:
                result = await db.execute(
    ...
    async def count_by_user_id(self, user_id: int) -> int:
        ...
        try:
            async with get_async_db() as db:
                result = await db.execute(
                    select(func.count()).select_from(ModelConnection).where(ModelConnection.user_id == user_id)
                )
    ...
    async def get_active_by_user_id(self, user_id: int, auto_set_first: bool = True) -> Optional[ModelConnection]:
        """根据用户ID获取活跃的模型连接
        ...
        try:
            async with get_async_db() as db:
                ...
                connection_result = await db.execute(
                    select(ModelConnection).where(ModelConnection.id == user.active_model_connections)
                )
                return cast(Optional[ModelConnection], connection_result.scalar_one_or_none())
        except Exception as e:
    ...
    async def set_active_by_user_id(self, user_id: int, connection_id: int) -> bool:
        ...
        try:
            async with get_async_db() as db:
                ...
                connection: Optional[ModelConnection] = connection_result.scalar_one_or_none()
                if not connection:
                    return False
