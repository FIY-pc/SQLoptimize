...
async def run_explain(state: SQLState ,sql: str, database: str) -> Tuple[bool, str]:
    """
    获取查询计划（MySQL）。
    ...
    try:
        ...
        plan_result = await mysql_utils.get_mysql_explain_plan(sql, database)
        if plan_result["success"]:
            ...
            logger.error(f"MySQL EXPLAIN失败: {plan_result.get('error', '未知错误')}")
            return False, f"MySQL EXPLAIN失败: {plan_result.get('error', '未知错误')}"
    except Exception as e:
        logger.error(f"MySQL EXPLAIN异常: {e}")
        return False, f"MySQL EXPLAIN异常: {e}"

async def run_explain_cost(state: SQLState, sql: str, database: str) -> Optional[float]:
    """
...
async def fetch_db_stats(state: SQLState, sql: str, database: str) -> Dict[str, Any]:
    """
    获取数据库统计信息
    ...
    try:
        ...
        ok, explain_json = await run_explain(state, sql, database)
        ...
        for tbl in normalized:
            res = await mysql_utils.get_mysql_table_statistics(tbl, database)
            if res.get("success"):
